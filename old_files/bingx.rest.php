...
class rbingx {
	...
	function praseParam($paramsMap) {
		if (!array_key_exists('timestamp', $paramsMap)) {
			$paramsMap['timestamp'] = round(microtime(true) * 1000);
		}
		ksort($paramsMap);
	...
	function api_set_leverage($symbol,$leverage) {
		try {
			...
			error_log("BingX api_set_leverage Error: " . $e->getMessage());
	...
	function api_set_margin_type($symbol,$marginType) { // margintype =  	ISOLATED, CROSSED
		try {
			...
			error_log("BingX api_set_margin_type Error: " . $e->getMessage());
			return ['code' => -1, 'msg' => 'An exception occurred: ' . $e->getMessage()];
		}
	}
	...
	function order_status($symbol, $orderid)
    {
        ...
        $order = $this->call("/openApi/swap/v2/trade/order", 1, $params, "GET");
        $result = json_decode(json_encode($order), true);

        // BingX'in cevabı data.order içinde gelir.
    ...
    function order_send($symbol, $side, $type, $amount, $price, $cls = 0)
    {
		...
		$o_keys['positionSide']="BOTH"; // HATA: Bu satır doğru positionSide mantığını eziyordu, kaldırıldı.
