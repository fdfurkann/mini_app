...
// Logları göstermek için yeni bir bileşen
const TradeLogs: React.FC<{ tradeId: number }> = ({ tradeId }) => {
  const { data: logs, isLoading, error } = useQuery<any[]>({
    queryKey: ['trade-logs', tradeId],
    queryFn: async () => {
      const response = await fetch(`${API_URL}/admin/user_signals/${tradeId}/logs`);
  ...
  if (isLoading) return <div className="p-4 text-center">Loglar yükleniyor...</div>;
  if (error) return <div className="p-4 text-center text-red-500">Hata: {(error as Error).message}</div>;
  if (!logs || logs.length === 0) return <div className="p-4 text-center">Bu işlem için log bulunamadı.</div>;

  return (
    <div className="p-2 bg-muted/50 text-xs font-mono">
      {logs.map((log, index) => (
        <div key={index} className="flex items-start">
          <span className="bg-blue-500 text-white px-1 rounded-sm mr-2" style={{ whiteSpace: 'nowrap' }}>
...
// API Anahtarı Detayları ve Düzenleme Formu için yeni bileşen
const ApiKeyDetails: React.FC<{ apiKeyId: number }> = ({ apiKeyId }) => {
  const queryClient = useQueryClient();
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState<Partial<ApiKey>>({});

  const { data: apiKey, isLoading, error } = useQuery<ApiKey>({
    queryKey: ['api-key-details', apiKeyId],
    queryFn: async () => {
      const response = await fetch(`${API_URL}/admin/api-keys/${apiKeyId}`);
      if (!response.ok) throw new Error('API anahtar detayları alınamadı');
      return response.json();
    },
    enabled: !!apiKeyId,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (updatedData: Partial<ApiKey>) => {
      const response = await fetch(`${API_URL}/admin/api-keys/${apiKeyId}`, {
        method: 'PUT',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['api-key-details', apiKeyId] });
      queryClient.invalidateQueries({ queryKey: ['member-details'] }); // Ana listeyi de yenile
  ...
  const fieldGroups = {
    "Temel Ayarlar": [
      { name: 'api_name', label: 'API Adı', type: 'text' },
      { name: 'api_key', label: 'API Key', type: 'text' },
      { name: 'api_secret', label: 'API Secret', type: 'password' },
      { name: 'api_type', label: 'Borsa', type: 'select', options: [{value: 1, label: 'Binance'}, {value: 2, label: 'Bybit'}, {value: 3, label: 'BingX'}] },
      { name: 'bot_room', label: 'Bot Odası ID', type: 'number' },
    ],
    "İşlem Ayarları": [
      { name: 'lotsize', label: 'Lotsize (USDT)', type: 'number' },
      { name: 'leverage', label: 'Kaldıraç', type: 'number' },
      { name: 'margin_type', label: 'Margin Tipi', type: 'select', options: [{value: 'ISOLATED', label: 'ISOLATED'}, {value: 'CROSSED', label: 'CROSSED'}] },
      { name: 'max_orders', label: 'Maks. Emir', type: 'number' },
    ],
    "Risk Yönetimi": [
      { name: 'stop_loss_settings', label: 'Stop Loss Stratejisi', type: 'select', options: [{value: 'none', label: 'Yok'}, {value: 'signal', label: 'Sinyal SL Kullan'}, {value: 'custom', label: 'Özel SL (%)'}] },
      { name: 'percent_loss', label: 'Özel SL Yüzdesi (%)', type: 'number' },
      { name: 'trail_stop', label: 'Trail Stop (TP Seviyesi)', type: 'number' },
      { name: 'break_even_level', label: 'Maliyete Çek (TP Seviyesi)', type: 'number' },
    ],
    "Kar Al Ayarları": [
      { name: 'take_profit', label: 'Kar Al Stratejisi', type: 'select', options: [{value: 'none', label: 'Yok'}, {value: 'signal', label: 'Sinyal TP Kullan'}, {value: 'custom', label: 'Özel TP (%)'}] },
      { name: 'percent_profit', label: 'Özel TP Yüzdesi (%)', type: 'number' },
      ...Array.from({ length: 10 }, (_, i) => ({ name: `tp${i + 1}`, label: `TP${i + 1} Kapatma Yüzdesi (%)`, type: 'number' })),
    ],
    "Durum": [
        { name: 'auto_trade', label: 'Otomatik İşlem', type: 'checkbox' },
        { name: 'status', label: 'Aktif', type: 'checkbox' },
    ]
  };
    ...
    <div className="p-4 bg-muted/50">
      <div className="flex justify-end mb-4">
        {!isEditing ? (
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => { setIsEditing(false); setFormData(apiKey); }}>
              <XCircle className="w-3 h-3 mr-2" /> İptal
            </Button>
            <Button size="sm" onClick={handleSave} disabled={updateMutation.isPending}>
              <Check className="w-3 h-3 mr-2" /> {updateMutation.isPending ? 'Kaydediliyor...' : 'Kaydet'}
            </Button>
          </div>
        )}
      </div>
      ...
      {Object.entries(fieldGroups).map(([groupTitle, fields]) => (
        <div key={groupTitle} className="mb-6">
          <h4 className="text-sm font-semibold mb-2 border-b pb-1">{groupTitle}</h4>
              ...
              <div key={field.name}>
                ...
                {field.type === 'select' ? (
                  <select
                    id={field.name}
                    name={field.name}
                    value={formData[field.name as keyof ApiKey] as string | number || ''}
                    onChange={handleInputChange}
                    disabled={!isEditing}
                    className="w-full text-sm h-8 p-1 border rounded bg-background"
                  >
                  ...
                  <div className="flex items-center h-8">
                    <input
                      ...
                      id={field.name}
                      name={field.name}
                      checked={!!formData[field.name as keyof ApiKey]}
                      onChange={handleInputChange}
                      disabled={!isEditing}
...
const ManageMember: React.FC = () => {
  ...
  const [expandedApiKeys, setExpandedApiKeys] = useState<Record<number, boolean>>({});
  ...
  const toggleApiKeyExpansion = (keyId: number) => {
  ...
  if (memberDataLoading) return <div className="p-6">{t('loading')}</div>;
            ...
            <TableHeader className="bg-muted/50">
              <TableRow>
                {isMobileView ? (
                  ...
                  <>
                    <TableHead className="w-[60px] px-2 py-2 text-xs">ID</TableHead>
                    <TableHead className="px-2 py-2 text-xs">Sembol</TableHead>
                    <TableHead className="px-2 py-2 text-xs">Yön</TableHead>
                    <TableHead className="px-2 py-2 text-xs text-right">Açılış Fiyatı</TableHead>
                    <TableHead className="px-2 py-2 text-xs text-right">Açılış Zamanı</TableHead>
                ...
                trades.map((trade) => (
                  <React.Fragment key={trade.id}>
                    ...
                    >
                      {isMobileView ? (
                        <>
                          <TableCell className="font-medium px-2 py-1.5">{trade.id}</TableCell>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>API Key (Kısmi)</TableHead>
              ...
              <TableBody>
                {apiKeys?.map((key) => (
                  <React.Fragment key={key.id}>
                    <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => toggleApiKeyExpansion(key.id)}>
                      <TableCell className="p-1.5">
                        {expandedApiKeys[key.id] ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                      </TableCell>
                      <TableCell className="font-medium">{key.api_name}</TableCell>
                      <TableCell>{key.api_key?.substring(0, 8)}...</TableCell>
                      <TableCell>{formatDate(key.created_at)}</TableCell>
                      <TableCell>
                        <Badge variant={key.status ? 'default' : 'secondary'}>
                          {key.status ? 'Aktif' : 'Pasif'}
                        </Badge>
                      </TableCell>
                      <td className="p-2 text-right">
                        {/* Silme butonu burada kalabilir */}
                      </td>
                    </TableRow>
                    {expandedApiKeys[key.id] && (
                      <TableRow>
                        <TableCell colSpan={6} className="p-0">
                          <ApiKeyDetails apiKeyId={key.id} />
                        </TableCell>
                      </TableRow>
                    )}
                  </React.Fragment>
