...
class rbinance
{
	...
	*/
    ...
    {
        $form = $this->call("/fapi/v1/leverageBracket", 1, [], "GET");
    ...
    function api_set_margin_type($symbol, $marginType)
    {
    ...
    {
        $form = $this->call("/fapi/v2/positionRisk", 1, [], "GET");
        $form = $this->obj2arr($form);
    ...
    {
        $o_keys = ["symbol" => $symbol, "orderId" => $orderid]; // ,"workingType"=>"CONTRACT_PRICE"
        ...
        if ( (is_object($order) && isset($order->code) && $order->code < 0) || (is_array($order) && isset($order['code']) && $order['code'] < 0) ) {
            $errorCode = is_object($order) ? $order->code : $order['code'];
            $errorMsg = is_object($order) ? $order->msg : $order['msg'];
            echo "order_delete(error): " . $errorCode . " " . $errorMsg . "\n";
            return $order;
        }
    ...
    function order_send($symbol, $side, $type, $amount, $price, $cls = 0)
    {
        ...
        if ($emri_kapat == 1) {
            ...
            if (isset($order['code']) && $order['code'] < 0) {
                echo "order_send(error): " .
                    ($order['code'] ?? 'UNKNOWN') .
                    " " .
                    ($order['msg'] ?? 'UNKNOWN ERROR') .
                    "\n";
                return $order;
            }
            if ($this->debug) {
