...
if ($aa["close"] > 0 && $aa["volume"] <= $aa["closed_volume"]) {
    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Position for signal ID {$signal_id} already closed.");
} elseif (
    ($admin_ok == 1 ||
        ($aa["open"] == 0 &&
    ...
    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: create_order called for signal ID {$signal_id}.");
    $bir_kere_ac = 1;
    ...
    if (
        ...
        if ($aa["sl"] > 0) {
            ...
            $new_sl = number_format(
                $sym["price"] * ((100 - $api["percent_loss"]) / 100),
                $sym["digits"],
                ".",
                ""
            );
        ...
        if ($new_sl > 0) {
            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Sending SL order (SELL) for {$signal["symbol"]} at {$new_sl}.");
            ...
            if ($sl_ticket["orderId"] != "") {
                ...
                $error_msg = str_replace("\"", "", $error_msg);
                // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: SL order failed. Error: {$error_code} - {$error_msg}");
                ...
                if ($acik_poz == 0) {
                    ...
                    foreach ($open_orders as $op1 => $op2) {
                        if (
                            ...
                            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Cancelling existing STOP_MARKET order {$op2["orderId"]} due to error -4130.");
                    ...
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: sticket cleared due to error -4130.");
                } else {
                    $sticket = -1;
                    ...
                    if (
                        ...
                        $sticket = $us_id;
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: SL order failed, orders or positions available. Setting sticket to {$us_id}.");
                    }
                    ...
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: SL order failed, user_signals updated with error and sticket.");
                }
            }
        }
    }
    ...
    if (
        ...
        if ($new_tp > 0 && $aa["tp_wait"] + $sl_tp_wait_seconds < time()) {
            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Sending TP order (SELL) for {$signal["symbol"]} at {$new_tp}.");
            ...
            if ($tp_ticket["orderId"] != "") {
                ...
                $error_msg = str_replace("'", "", $error_msg);
                $error_msg = str_replace("\"", "", $error_msg);
                // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: TP order failed. Error: {$error_code} - {$error_msg}");
                ...
                if ($acik_poz == 0) {
                    ...
                    foreach ($open_orders as $op1 => $op2) {
                        if (
                            ...
                            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Cancelling existing TAKE_PROFIT_MARKET order {$op2["orderId"]} due to error -4130.");
                            $cancel_stop = $binance->order_delete(
                                $symbol,
                                $op2["orderId"]
                            );
                    ...
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: tticket cleared due to error -4130.");
                } else {
                    $tticket = -1;
                    ...
                    if (
                        ...
                        $tticket = $us_id;
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: TP order failed, orders or positions available. Setting tticket to {$us_id}.");
                    }
                    ...
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: TP order failed, user_signals updated with error and tticket.");
                }
            }
        }
        ...
        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: create_limit_tp called for signal ID: {$signal_id}.");
    }

    // START: NEW TRAIL STOP & BREAK EVEN LOGIC
    $current_tp_level = isset($signal["tp_hit"])
        ? (int) $signal["tp_hit"]
        : 0;
    ...
    if ($current_tp_level > $user_tp_hit) {
        ...
        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: TP hit level increased from {$user_tp_hit} to {$current_tp_level}.");
        $new_sl_details = null;
        ...
        if (isset($api["trail_stop"]) && $api["trail_stop"] > 0) {
            ...
            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Trail Stop enabled with index {$trail_idx}.");
        ...
        if (
            ...
            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Break Even enabled with level {$break_even_idx}.");
            if ($current_tp_level >= $break_even_idx) {
                ...
                // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Break Even condition met. New SL candidate: {$aa["open"]}.");
            }
        }
        ...
        if ($new_sl_details) {
            ...
            if ($new_sl > 0) {
                if ($signal["direction"] == "LONG") {
                    if ($current_sl == 0 || $new_sl > $current_sl) {
                        ...
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: SL should be updated (LONG): current {$current_sl}, new {$new_sl}.");
                    }
                } elseif ($signal["direction"] == "SHORT") {
                    if ($current_sl == 0 || $new_sl < $current_sl) {
                        $should_update_sl = true;
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: SL should be updated (SHORT): current {$current_sl}, new {$new_sl}.");
                    }
                }
            }
            ...
            if ($should_update_sl) {
                if (!empty($aa["sticket"]) && $aa["sticket"] > 0) {
                    try {
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Deleting existing SL order: {$aa["sticket"]}.");
                        $binance->order_delete($symbol, $aa["sticket"]);
                    } catch (Exception $e) {
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Error deleting existing SL order: {$e->getMessage()}");
                    }
                }
                ...
                if ($signal["direction"] == "LONG" && $new_sl_price < $sym["price"]) {
                    ...
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: New SL price {$new_sl_price} is not valid against current price {$sym["price"]}. Skipping SL order.");
                }

                if ($is_safe_to_place_sl) {
                    for ($attempt = 1; $attempt <= 3; $attempt++) {
                        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Attempt {$attempt} to place new SL order at {$new_sl_price}.");
                        try {
                            ...
                            if (!empty($new_stop_order["orderId"])) {
                                ...
                                $success = true;
                                // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: New SL order placed successfully, orderId: {$order_id_for_db}.");
                                $break_buy = 1;
                                break;
                            ...
                            // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: Error placing new SL order: {$e->getMessage()}");
                        }
                ...
                $notification_str = "";
                if ($success) {
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: New SL order successful. Reason: {$new_sl_details["reason"]}.");
                    ...
                    // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: New SL order failed after 3 attempts. Error: {$error_msg}");
                    $notification_str =
                ...
                $my->query(
                    "UPDATE user_signals SET sl='{$new_sl_price}', sticket='{$order_id_for_db}' WHERE id='{$signal_id}'"
                );
                // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: user_signals updated with new SL price: {$new_sl_price} and sticket: {$order_id_for_db}.");
                $aa["sl"] = $new_sl_price;
        ...
        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: user_signals updated with new tp_hit: {$current_tp_level}.");
        $aa["tp_hit"] = $current_tp_level;
    }

    // Pozisyon kapatma kontrolü
    $pozisyon_kapatildi = false;
    ...
    if (
        ...
        // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: SL triggered for signal ID {$signal_id} at price {$sym["price"]}. Closing order.");
        close_order(
            $signal_id,
            $aa["sl"],
            "SL",
    ...
    if (!$pozisyon_kapatildi) {
        for ($i = 10; $i >= 1; $i--) {
            if (
                ...
                // print_log($my, $s_id, $user_id, $us_id, $channel, "manage_buy: TP{$i} hit for signal ID {$signal_id} at price {$sym["price"]}. Closing order.");
                // Burada pozisyonu kapatmak yerine sadece bildirim gönderilebilir veya
                // kullanıcı ayarlarına göre belirli bir yüzde kapatılabilir.
                // Şimdilik sadece logluyoruz ve döngüden çıkıyoruz.
                // Gerekirse close_order çağrısı eklenebilir.
                break; // İlk ulaşılan TP seviyesinde işlemi sonlandır
            }
        }
    }
}
