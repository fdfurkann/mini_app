...
class rbybit
{
    ...
    private function signedRequest($url, $params = [], $method = "GET")
    {
        ...
        $signature = hash_hmac(
            "sha256",
            $timestamp . $this->auth["key"] . "5000" . $query,
            $this->auth["secret"]
        );
        if ($method == "POST") {
            $opt = [
	...
	*/
    ...
    {
        $form = $this->call("/v5/account/upgrade-to-uta", 1, [], "POST");
        $form = $this->obj2arr($form);
        $form1 = [];
        $form1["code"] = $form["retCode"];
        $form1["msg"] = $form["retMsg"];
    ...
    function api_switch_mode($symbol, $trading_mode)
    {
    ...
    function api_set_margin_mode($marginType)
    {
        // margintype = ISOLATED, CROSSED
        // Bybit v5: Account level margin mode -> ISOLATED_MARGIN veya REGULAR_MARGIN
        $mode = strtoupper($marginType);
        if ($mode === "ISOLATED") {
            $marginModeParam = "ISOLATED_MARGIN";
        } elseif ($mode === "CROSSED") {
            $marginModeParam = "REGULAR_MARGIN";
        } else {
            return [
                'code' => -1,
                'msg' => 'Invalid marginType. Use ISOLATED or CROSSED.'
            ];
        }
        ...
        $form = $this->call(
            ...
            1,
            ["setMarginMode" => $marginModeParam],
            "POST"
        );
        $form = $this->obj2arr($form);
        $form1 = [];
        $form1["code"] = $form["retCode"] ?? ($form["code"] ?? -1);
        $form1["msg"] = $form["retMsg"] ?? ($form["msg"] ?? json_encode($form));
        return $form1;
    }

    function api_set_margin_type($symbol, $marginType, $leverage = 20)
    {
        // margintype = ISOLATED, CROSSED
        // Bybit UTA'da sembol bazlı isolated/cross değişimi için /v5/position/switch-isolated kullanılmalıdır.
        $mode = strtoupper($marginType);
        if ($mode == "ISOLATED") {
            $tradeMode = 1;
        } elseif ($mode == "CROSSED") {
            ...
            return [
                'retCode' => -1,
                'retMsg' => 'Invalid marginType provided. Use ISOLATED or CROSSED.'
            ];
        }
        ...
        $form1["code"] = $form["retCode"] ?? ($form["code"] ?? -1);
        $form1["msg"] = $form["retMsg"] ?? ($form["msg"] ?? json_encode($form));
        $form1["retCode"] = $form["retCode"] ?? ($form["code"] ?? null);
        $form1["retMsg"] = $form["retMsg"] ?? ($form["msg"] ?? null);
        return $form1;
    }
    ...
    {
        // margintype =  	ISOLATED, CROSSED
        $form = $this->call("/sapi/v1/account/apiRestrictions", 1, [], "GET");
        $form = $this->obj2arr($form);
        return $form;
    }
    ...
    function api_set_position_mode($dualSidePosition)
    {
    ...
    function api_set_multi_asset_mode($multiAssetsMargin)
    {
        // true = multi asset mode , false = single asset mode
        $form = $this->call(
    ...
    function fapi_historicalTrades()
    {
        // true = multi asset mode , false = single asset mode
        $form = $this->call(
    ...
    function open_positions($symbol = "")
    {
    ...
    function get_account_info($symbol = "")
    {
        // true = multi asset mode , false = single asset mode
        $form = $this->call("/v5/account/info", 1, [], "GET");
        $form = $this->obj2arr($form);
        return $form;
    }
    ...
    {
        trade_log(
            "order_send(symbol:$symbol,side:$side,type:$type,amount:$amount,price:$price,cls:$cls)"
        );
        ...
        if ($emri_kapat == 1) {
            ...
            trade_log(" order result:" . print_r($order, 1));
            ...
            if (($type == "TP" or $type == "SL") && $order["retMsg"] == "OK") {
                ...
                foreach ($ords as $a1 => $b1) {
                    if (
                        $b1["stopOrderType"] == "TakeProfit" and
                        $type == "TP"
                    ) {
                        $n_order["orderId"] = $b1["orderId"];
                        break;
                    } elseif (
                        $b1["stopOrderType"] == "StopLoss" and
                        $type == "SL"
                    ) {
                        $n_order["orderId"] = $b1["orderId"];
                        break;
