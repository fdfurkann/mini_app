...
class rbingx {
    ...
    // get_exchange fonksiyonu (BingX için contracts endpointi kullanılacak)
    async get_exchange() {
        let form = await this.call('/openApi/swap/v2/quote/contracts', 0, {}, "GET");
        form = this.obj2arr(form);
        return form.data;
    }

    // get_leverage fonksiyonu (get_leverage_2 ile aynı, eski get_leverage silindi)
    async get_leverage() {
    ...
    async order_send(symbol, side, type, amount, price, cls = 0) {
        ...
        if (type == "MARKET") {
            ...
            // BingX için SL: STOP_MARKET
            o_keys['quantity'] = amount + "";
            o_keys['type'] = "STOP_MARKET";
            o_keys['timeInForce'] = "GTC";
            o_keys['stopPrice'] = price + "";
        } else if (type == "TP") {
            // BingX için TP: TAKE_PROFIT_MARKET
            o_keys['quantity'] = amount + "";
    ...
    prepare_order(symbol, side, type, amount, price, cls = 0) {
        ...
        if (price !== undefined) price = price.toString();
        return o_keys;
    }
    ...
    async set_symbol(symbol, exchangeInfo) {
        ...
        // BingX için doğru precision ve miktar değerlerini ata
        ...
        this.stepSize = symbolInfo.size ? parseFloat(symbolInfo.size) : 0.0001;
        this.minQty = symbolInfo.tradeMinQuantity ? parseFloat(symbolInfo.tradeMinQuantity) : 0.0001;
        this.minUSDT = symbolInfo.tradeMinUSDT ? parseFloat(symbolInfo.tradeMinUSDT) : 0;
        this.max_lot = 0;
        this.max_leverage = 0;
