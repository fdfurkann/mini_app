...
class rbybit {
    ...
    async get_exchange() {
        let form = await this.call('/v5/market/instruments-info', 0, { category: 'linear' }, 'GET');
        form = this.obj2arr(form);
        if (!form || !form.result || !form.result.list) {
            console.log('Bybit get_exchange: API yanıtı beklenen formatta değil:', form);
            return [];
        }
    ...
    async order_send(symbol, side, type, amount, price, cls = 0) {
        ...
        // amount ve price düzeltmeleri (binance_rest.js ile aynı mantık)
        if (this.stepSize) {
    ...
    async set_symbol(symbol, exchangeInfo) {
        ...
        } else if (exchangeInfo && Array.isArray(exchangeInfo.data)) {
            symbolList = exchangeInfo.data;
        } else if (exchangeInfo && Array.isArray(exchangeInfo.result)) {
            symbolList = exchangeInfo.result;
        } else if (Array.isArray(exchangeInfo)) {
            symbolList = exchangeInfo;
        }
        // Bybit yeni API: sembol listesi doğrudan array
        const symbolInfo = symbolList.find(s => s.symbol === symbol || s.symbol === symbol.replace('USDT', '-USDT'));
        if (!symbolInfo) throw new Error('Sembol bulunamadı!');
        this.symbolInfo = symbolInfo;
        // Bybit için doğru precision ve miktar değerlerini ata
        this.digits = symbolInfo.pricePrecision || 2;
        this.vdigits = symbolInfo.quantityPrecision || 3;
        this.tickSize = symbolInfo.pricePrecision ? Math.pow(10, -symbolInfo.pricePrecision) : 0.0001;
        this.stepSize = symbolInfo.lotSizeFilter ? parseFloat(symbolInfo.lotSizeFilter.qtyStep) : 0.0001;
        this.minQty = symbolInfo.lotSizeFilter ? parseFloat(symbolInfo.lotSizeFilter.minOrderQty) : 0.0001;
        this.max_lot = symbolInfo.lotSizeFilter ? parseFloat(symbolInfo.lotSizeFilter.maxOrderQty) : 0;
        this.max_leverage = symbolInfo.leverageFilter ? parseFloat(symbolInfo.leverageFilter.maxLeverage) : 0;
        return symbolInfo;
    }

    async close_position(symbol) {
        try {
            const positions = await this.open_positions(symbol);
    ...
    // positions fonksiyonu (open_positions ile aynı)
    async positions(symbol = "") {
        return await this.open_positions(symbol);
    }

    // close_order fonksiyonu (binance_rest.js ile aynı mantık)
    async close_order(symbol) {
        return await this.close_position(symbol);
    }
}
