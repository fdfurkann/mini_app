...
function create_order(
    ...
    try {
        ...
        print_log($my, $s_id, $user_id, $us_id, $channel, "Fetched API details".print_rr($api,true));   
        ...
        if (isset($p_risk[$symbol]) && $p_risk[$symbol] != 0) {
            ...
            // TP emir
            if ($tprice > 0 && isset($api["take_profit"]) && $api["take_profit"] != "none" && $api["take_profit"] != "signal") {
        ...
        $error_message = "CRITICAL ERROR in create_order for signal {$sid}: " . $e->getMessage() . " on line " . $e->getLine();
        print_log($my, $s_id, $user_id, $us_id, $channel, $error_message);

        // Sinyali veritabanında hatalı olarak işaretle
        $my->query(
            "UPDATE user_signals SET status=3, event='" . $my->real_escape_string($error_message) . "' WHERE id='" . $sid . "'"
        );
    }
}
?>
